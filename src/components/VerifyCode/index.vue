...
const makeCode = (len = 4) => {
  ...
  const identifyCodes =
    "123456789abcdefjhijkinpqrsduvwxyzABCDEFGHJKLMNPQRSTUVWXYZ";
...
// 模拟校验验证码是否正确
const isEqual = (code: string) => {
  if (!code || !verifyCode.value) {
    return false;
  }
  return code.toLocaleLowerCase() === verifyCode.value.toLocaleLowerCase();
};
// 定义暴露接口
defineExpose({ refreshCode, isEqual });
